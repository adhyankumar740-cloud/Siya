...
def clean_message_for_logging(message: str, bot_username: str) -> str:
    ...
    cleaned_message = re.sub(r'siya\s*(ko|ka|se|ne|)\s*', '', cleaned_message, flags=re.IGNORECASE) # 'pihu' se 'siya' kiya
...
async def get_bot_response(user_message: str, chat_id: int, bot_username: str, should_use_ai: bool, update: Update) -> str:
    ...
    date_time_patterns = [
        ...
        r'siya abhi ka time batao' # 'pihu' se 'siya'
    ]
...
# --- NOTE: This function is no longer called by process_message to save quota ---
async def is_message_for_siya(user_message: str) -> bool: # Naam Pihu se Siya kiya
    ...
    try:
        ...
        logger.error(f"Error checking if message is for Siya: {e}") # Siya
        return False
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    add_to_group_url = f"https://t.me/The_Ziya_bot?startgroup=true" # Pihu -> Siya
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    is_mention = f"@{bot_username.lower()}" in user_message.lower() or "siya" in user_message.lower() # 'pihu' se 'siya'
    
    should_use_ai = is_private_chat or is_reply_to_bot or is_mention
    
    # --- FIX: Removed AI-based fallback check to save API quota ---
    # if not should_use_ai:
    #     if await is_message_for_siya(user_message):
    #         should_use_ai = True
    #     else:
    #         return
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    about_text = (
        "❤️ **About Siya** ❤️\n\n" # Siya
        "I am Siya, an AI assistant . My purpose is to chat with you and provide answers to your questions.\n\n" # Siya
